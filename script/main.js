$(document).ready(function () {
    ...
    $(document).on("contextmenu", ".draggable", function (e) {
        ...
        context_render += "<td class='ctx-read-track'><img src='../imgs/media23.png' style='margin-right: 5px;'>Lecture</td>";
        context_render += "</tr>";
        context_render += "<tr id='ctx'>";
        context_render += "<input type='hidden' id='track-id-temp' value='" + $(this).find('#ply-index').val() + "' />";
        context_render += "<input type='hidden' id='ply-id-temp' value='" + $(this).find('#ply-id').val() + "' />";
        context_render += "<td class='ctx-delete-track'>Delete " + vTrack.getTrackTitle($(this).find('#track-network').val(), $(this).find('#track-id').val()) + "</td>";
        context_render += "</tr>";
        ...
        $(".ctx-delete-track").click(function () {
            console.log('id : ' + $("#ctx").find("#track-id-temp").val());
            var plmg = new playlistManager();

            plmg.remove($("#ctx").find('#track-id-temp').val(), $("#ctx").find('#ply-id-temp').val());

            $(".context-menu").hide(100);
            showUserPlaylists();
        });
        $(".ctx-read-track").click(function(){
            start($("#ctx-r").find("#track-id-temp").val(), $("#ctx-r").find("#ply-id-temp").val());
        })
    });
